...
```{r}
Fishies=filter(fishD, Species=="Perch")
...
```{r}
Fishies=filter(fishD, Species=="Bream")
Fishies = select(Fishies, -Species)
pairs(Fishies)
```
...
```{r}
Fishies=filter(fishD, Species=="Smelt")
Fishies = select(Fishies, -Species)
pairs(Fishies)
```
...
```{r}
fishPerch=lm(Weight~Length2+Width,data=Fishies)
summary(fishPerch)
```
...
```{r}
fishBream=lm(Weight~Length2+Width,data=Fishies)
summary(fishBream)
```
...
```{r}
fishSmelt=lm(Weight~Length2+Width,data=Fishies)
summary(fishSmelt)
```
